...
AT$SCAN | Scan for nearby 2.4 GHz WiFi networks. Lists one access point per line as `<index> <ssid><TAB><sec>` (1-based, de-duplicated by SSID keeping the strongest signal, hidden SSIDs skipped), then `OK`. `<sec>` is `O` for an open network or `S` for a secured one; the TAB keeps it backward compatible (a host ignoring the rest of the line still gets the SSID). The 1-based index is meant to drive a numbered menu whose chosen SSID is fed back via `AT$SSID=`.
